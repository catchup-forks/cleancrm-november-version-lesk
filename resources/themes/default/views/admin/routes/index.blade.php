    ...
    <div class='col-md-12'>
      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          ...
          &nbsp;
          <a class="btn btn-default btn-sm" href="{!! route('admin.routes.create') !!}"
             title="{{ trans('admin/routes/general.action.create') }}">
            <i class="fa fa-plus-square"></i>
          </a>
          &nbsp;
          <a class="btn btn-default btn-sm" href="{!! route('admin.routes.load') !!}"
             title="{{ trans('admin/routes/general.action.load-routes') }}">
            <i class="fa fa-refresh"></i>
          </a>
          &nbsp;
          <a class="btn btn-default btn-sm" href="#"
             onclick="document.forms['frmRouteList'].action = '{{ route('admin.routes.enable-selected') }}';  document.forms['frmRouteList'].submit(); return false;"
             title="{{ trans('admin/routes/general.action.enable-selected') }}">
            <i class="fa fa-check-circle-o"></i>
          </a>
          &nbsp;
          <a class="btn btn-default btn-sm" href="#"
             onclick="document.forms['frmRouteList'].action = '{{ route('admin.routes.disable-selected') }}';  document.forms['frmRouteList'].submit(); return false;"
             title="{{ trans('admin/routes/general.action.disable-selected') }}">
            <i class="fa fa-ban"></i>
          </a>
          &nbsp;
          <a class="btn btn-default btn-sm" href="#"
             onclick="document.forms['frmRouteList'].action = '{{ route('admin.routes.save-perms') }}';  document.forms['frmRouteList'].submit(); return false;"
             title="{{ trans('admin/routes/general.action.save-perms-assignment') }}">
            ...
            <table class="table table-hover">
              ...
              @foreach($routes as $route)
                <tr>
                  ...
                  @endif
                  <td>
                    <a href="{!! route('admin.routes.edit', $route->id) !!}" title="{{ trans('general.button.edit') }}"><i
                        class="fa fa-pencil-square-o"></i></a>

                    @if ( $route->enabled )
                      <a href="{!! route('admin.routes.disable', $route->id) !!}"
                         title="{{ trans('general.button.disable') }}"><i class="fa fa-check-circle-o enabled"></i></a>
                    @else
                      <a href="{!! route('admin.routes.enable', $route->id) !!}"
                         title="{{ trans('general.button.enable') }}"><i class="fa fa-ban disabled"></i></a>
                    @endif

                    <a href="{!! route('admin.routes.confirm-delete', $route->id) !!}" data-toggle="modal"
                       data-target="#modal_dialog" title="{{ trans('general.button.delete') }}"><i
